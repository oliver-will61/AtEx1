...
router.post('/login', async (req, res) => {
    ...
    const {email, senha} = req.body  // Dados do login recebidos no body da requisição
    ...
    try {
        ...
        const [rows] = await connetion.execute('SELECT email, senha FROM usuarios WHERE email = ?', [email])
        ...
        const senhaValida = await bcrypt.compare(senha, usuario.senha) // Compara a senha inserida com o hash armazenado
